...
| [`save-that`](save-that/SKILL.md) | Immediate mid-conversation capture — triggered by "save that"/"remember that" — instead of waiting for end-of-session wrap-up. Routes to the current repo's own wiki/SESSION-LOG (project-specific) and/or Claude Code's cross-session memory system (standing user preferences/feedback that should follow across projects), whichever actually applies. Built 2026-08-15. |
